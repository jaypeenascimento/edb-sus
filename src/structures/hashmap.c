...
void insertPatient(HashTable *hashTable, char *id, char *name, int age,
    ...
    while (hashTable->table[index] != NULL) {
        ...
        if (index == startIndex) {
            logMessage(logFile, "[ERROR] Tabela hash cheia, não foi possível inserir o paciente %s\n", id);
            return;
        }
    }
    ...
    logMessage(
        logFile,
        "Paciente inserido: [ID: %s, Nome: %s, Idade: %s, Sexo: %s, CPF: %s, Prioridade: %s, Atendido: %s] no índice %d\n",
        idNode->info,
        nameNode->info,
        ageNode->info,
        genderNode->info,
